...
export default function Layout({ children, userId, currentUser, onAddProcessTitle, processTitles }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newProcessName, setNewProcessName] = useState('');
  const [processDependency, setProcessDependency] = useState('');

  // Get authentication functions from context
  const { signInWithGoogle, handleSignOut } = useContext(FirebaseContext) || {};
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-300">
                  {currentUser.providerData.length > 0 
                    ? (currentUser.displayName || currentUser.email || `User: ${userId?.slice(0, 8)}...`)
                    : `Guest User (${userId?.slice(0, 8)}...)`
                  }
                </span>
              </div>
              <button
